...
class ResidualBlock(nn):
    def __init__(self, in_channels,out_channels, stride=1):
        ...
        # Downsampling
        if self.shortcut:
            nn.Sequential(nn.Conv2d(in_channels,out_channels,1, stride=stride, bias=false ,nn.BatchNorm2d(outchannels)))
    ...
    class AudioCNN(nn.Module):
        def __init__(self,num_classes=50):
            ...
            self.conv1=nn.Sequential(
                nn.Conv2(1,64,7,stride=2,padding=3,bias=False), nn.BatchNorm2d(64), nn.ReLU(inplace=True),nn.MaxPool2d(3,stride=2,padding=1)
            )     
        ...
        def forward(self,x):
            ...
            x=self.avgpool(x)
            # reshape the tensor without changing data ("transformation")
            x=x.view(x.size(0),-1)
